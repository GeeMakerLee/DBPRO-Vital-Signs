...
/*
' creates two streams among the given input parameters (here: power and temperature)
 */
    ...
    public void run(SourceContext<MonitoringEvent> sourceContext) throws Exception {
        while (running) {
            ...
            if (random.nextDouble() >= temperatureRatio) {
                ...
                monitoringEvent = new PowerEvent(ts, rackId, power);
            } else {
                double temperature = random.nextGaussian() * temperatureStd + temperatureMean;
                monitoringEvent = new TemperatureEvent(ts, rackId, temperature);
            }
